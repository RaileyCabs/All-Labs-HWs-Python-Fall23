...
class Book:
    ...
    def checkout(self):
        self.borrowed = True
        return f"Book {self.Title} has been checked out."

    def checkin(self):
        self.borrowed = False
        return f"Book {self.Title} has been checked in."
...
class User:
    def __init__(self, Name: str, ID: int, borrowedBooks: list = []):
        self.Name = Name
        self.ID = ID
    ...
    def return_book(self, book):
        ...
        book.borrowed = False
        return f"Book {book.Title} has been checked in."
...
class Library:
    ...
    def export_users_to_csv(self, filename):
        ...
        with open(filename, 'w', newline='') as csvfile:
            ...
            writer.writeheader()
            for user in self.users:
                borrowed_books_titles = [book.Title for book in user.borrowedBooks]
...
def main():
    ...
    while True:
        ...
        try:
            ...
            if choice == 'a':
                # Implement option to add books
                library.add_book()
            elif choice == 'b':
                # Implement option to add users
                library.add_user()
            # Add other conditions for different menu options...
            elif choice == 'z':
                break  # Exit the loop if 'z' is selected
            else:
