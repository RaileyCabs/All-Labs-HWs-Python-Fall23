#  ________  ________  ________        ___  _______   ________ _________        ________
# |\   __  \|\   __  \|\   __  \      |\  \|\  ___ \ |\   ____\\___   ___\     |\_____  \
# \ \  \|\  \ \  \|\  \ \  \|\  \     \ \  \ \   __/|\ \  \___\|___ \  \_|     \|____|\ /_
#  \ \   ____\ \   _  _\ \  \\\  \  __ \ \  \ \  \_|/_\ \  \       \ \  \            \|\  \
#   \ \  \___|\ \  \\  \\ \  \\\  \|\  \\_\  \ \  \_|\ \ \  \____   \ \  \          __\_\  \
#    \ \__\    \ \__\\ _\\ \_______\ \________\ \_______\ \_______\  \ \__\        |\_______\
#     \|__|     \|__|\|__|\|_______|\|________|\|_______|\|_______|   \|__|        \|_______|



# Author: JAMES RAILEY CABRERA
# CIST2110-001-Project-3 Library Management System (LMS)
# Project 3 will implement a library management system (LMS) that will allow users to manage books, users, and a library to manage collection of books and users.
# The LMS will be menu driven and will allow users to add, delete, and update books and users.
# Users will also be able to borrow and return books.
# The LMS will also allow users to search for books and users.

# ENABLE WORD WRAP TO MAKE THINGS EASIER TO READ:
# VIEW (at the top) -> WORD WRAP

# Import statements:


# Project outline and requirements:

# OUTLINE - The LMS will consist of the following classes and methods:

# 1. Create a Book class that has the following attributes (create a __init__ method)):
#    a. ISBN (int)
#    b. Title (string)
#    c. Author (string)
#    d. borrowed (boolean) - this should not be passed in as a parameter, it should be set to False by default
# USE SELF IN THE __INIT__ METHOD TO CREATE THESE ATTRIBUTES

# Methods:
#    a. __str__ (returns a string representation of the book using the following format: ISBN: <ISBN>, Title: <Title>, Author: <Author>, Borrowed: <Borrowed>)
#    b. checkout - sets borrowed to True and returns a message that the book has been checked out
#    c. checkin - sets borrowed to False and returns a message that the book has been checked in
#    d. isBorrowed - returns True if the book is borrowed and False if the book is not borrowed


# 2. Create a User class that has the following attributes (create a __init__ method)):
#    a. Name (string)
#    c. ID (int)
#    d. borrowedBooks (list of books) - this should not be passed in as a parameter, it should be set to an empty list by default
# USE SELF IN THE __INIT__ METHOD TO CREATE THESE ATTRIBUTES

# Methods:
#    a. __str__ (returns a string representation of the user using the following format: Name: <Name>, ID: <ID>, Borrowed Books: <Borrowed Books>)
#    b. borrow_book - adds the book to the borrowedBooks list, updates the isBorrowed attribute of the book to True, and returns a message that the book has been checked out (should take a book as a parameter)
#    c. return_book - removes the book from the borrowedBooks list, updates the isBorrowed attribute of the book to False, and returns a message that the book has been checked in (should take a book as a parameter)


    
# 3. Create a Library class that has the following attributes (create a __init__ method)):
#    a. books (list of books)
#    b. users (list of users)
# USE SELF IN THE __INIT__ METHOD TO CREATE THESE ATTRIBUTES

# Methods:
#    a. __str__ (returns a string representation of the library using the following format: Books: <Books>, Users: <Users>)
#    b. add_book - adds a book to the books list (should take a book as a parameter)
#    c. add_user - adds a user to the users list (should take a user as a parameter)
#    d. find_book - returns the book with the given ISBN (should take an ISBN as a parameter)
#    e. find_user - returns the user with the given ID (should take an ID as a parameter)
#    f. export_books_to_csv - exports the books list to a csv file (should take a filename as a parameter)
#       The csv file should have the following format: ISBN,Title,Author,Borrowed
#       The csv.DictWriter class is very useful for this: https://docs.python.org/3/library/csv.html#csv.DictWriter
#    g. export_users_to_csv - exports the users list to a csv file (should take a filename as a parameter)
#       This will be similar to the export_books_to_csv method but there is a slight difference with the borrowedBooks attribute if you get stuck this code might help:
#       borrowed_books_titles = [book.title for book in user.borrowed_books]
#       Use that and pythons .join method to create a string of the borrowed books titles



# 4. Create a menu that will allow users to:
#    a. Add books
#    b. Add users
#    c. Delete books
#    d. Delete users
#    g. Borrow books
#    h. Return books
#    i. Search books
#    j. Check if book is available
#    k. Search users
#    l. Export books to csv
#    m. Export users to csv
#    z. Exit

# REQUIREMENTS:
# 1. You should be doing error checking on all user input (make sure the user enters a valid ISBN, ID, etc.) and handle any errors appropriately (i.e. if the user enters an invalid ISBN, ask them to enter a valid ISBN)
# 2. You should be using try except blocks to handle any errors
# 3. You should be using the classes and methods outlined above with the exact names and parameters
# 4. You should be using the menu to call the appropriate methods
# 5. There is a Project3Tests.py file that will help you test your code. You should be able to run that file and pass all the tests.
#    Remember to run pytest use the following command in the terminal: pytest Project3Tests.py
# 6. The Project3Tests.py file is missing 2 tests. test_user_return and test_library_find_user. You will need to implement those tests and ensure they pass.
# 7. In your main method you should create a library object first to use for the rest of the program. You should not be creating a new library object every time you call a method. (Similar to the Store object in Lab 11)
# 8. In your main method you should be using a while loop to keep the program running until the user chooses to exit.

# IMPORTANT: You will only have 1 submission for this project so make sure you test your code thoroughly before submitting.

# You will be graded on the following:
# 1. Did you follow the project outline and requirements?
# 2. Does your code run without errors?
# 3. Did you use try except blocks to handle errors?
# 4. Did you use the classes and methods outlined above with the exact names and parameters?
# 5. Did you use the menu to call the appropriate methods?
# 6. Did you include docstrings for all classes and methods?
# 7. Did you include type hints for all methods?
# 8. Did your pytests for the test_user_return and test_library_find_user work?


import streamlit as st
import csv

class Book:
    '''Class representing a book in the library
    
    Attributes:
        isbn (int): The ISBN of the book
        title (str): The title of the book
        author (str): The author of the book
        borrowed (bool): True if the book is borrowed, False otherwise
    '''
    
    def __init__(self, title:str, author:str, isbn:int):
        self.isbn = isbn
        self.title = title
        self.author = author
        self.borrowed = False

    def __str__(self) -> str:

        """Return a string representation of the book."""

        return f"ISBN: {self.isbn}, Title: {self.title}, Author: {self.author}, Borrowed: {self.borrowed}"

    def check_out(self) -> str:
        '''Check out the book and return a message.'''
        if not self.borrowed:
            self.borrowed = True
            return f"Book {self.title} checked out successfully."
        else:
            return f"Book {self.title} is already checked out."

    def check_in(self) -> str:

        '''Check in the book and return a message.'''

        if self.borrowed:
            self.borrowed = False
            return f"Book {self.title} checked in successfully."
        else:
            return f"Book {self.title} is not checked out."

class User:
    '''Class representing a user in the library
    
    Attributes:
        name (str): The name of the user
        member_id (int): The member ID of the user
        borrowed_books (list): The list of books borrowed by the user
        '''
    def __init__(self, name:str, member_id:int):
        self.name = name
        self.member_id = member_id
        self.borrowed_books = []

    def __str__(self) -> str:
        borrowed_books_titles = [book.title for book in self.borrowed_books]
        return f"Name: {self.name}, ID: {self.member_id}, Borrowed Books: {borrowed_books_titles}"

    def borrow_book(self, book:Book) -> str:

        '''Borrow the book and return a message.'''

        if not book.borrowed:
            self.borrowed_books.append(book)
            book.borrowed = True
            return f"Book {book.title} borrowed successfully."
        else:
            return f"Book {book.title} is already borrowed."

    def return_book(self, book:Book) -> str:

        '''Return the book and return a message.'''

        if book in self.borrowed_books:
            self.borrowed_books.remove(book)
            book.borrowed = False
            return f"Book {book.title} returned successfully."
        else:
            return f"You do not have book {book.title} borrowed."

class Library:
    '''Class representing a library 
        Attributes: 
        books (list): The list of books in the library
        users (list): The list of users in the library
    '''
    def __init__(self):
        self.books = []
        self.users = []

    def __str__(self) -> str:

        '''Return a string representation of the library.'''

        return f"Books: {len(self.books)}, Users: {len(self.users)}"

    def add_book(self, book):

        '''Add a book to the library.'''

        self.books.append(book)

    def add_user(self, user:User):

        '''Add a user to the library.'''

        self.users.append(user)

    def find_book(self, isbn:int):

        '''Find a book by ISBN and return it.'''

        for book in self.books:
            if book.isbn == isbn:
                return book
        return None

    def find_user(self, member_id:int)-> User:

        '''Find a user by member ID and return it.'''

        for user in self.users:
            if user.member_id == member_id:
                return user
        return None

    def export_books_to_csv(self, filename:str):

        '''Export the books list to a csv file (should take a filename as a parameter)'''

        with open(filename, mode='w', newline='') as file:
            writer = csv.DictWriter(file, fieldnames=['ISBN', 'Title', 'Author', 'Borrowed'])
            writer.writeheader()
            for book in self.books:
                writer.writerow({'ISBN': book.isbn, 'Title': book.title, 'Author': book.author, 'Borrowed': book.borrowed})

    def export_users_to_csv(self, filename:str):

        '''Export the users list to a csv file (should take a filename as a parameter)'''

        with open(filename, mode='w', newline='') as file:
            writer = csv.DictWriter(file, fieldnames=['Name', 'ID', 'Borrowed Books'])
            writer.writeheader()
            for user in self.users:
                borrowed_books_titles = [book.title for book in user.borrowed_books]
                writer.writerow({'Name': user.name, 'ID': user.member_id, 'Borrowed Books': ', '.join(borrowed_books_titles)})

    def delete_book(self, isbn:int):

        '''Delete a book by ISBN.'''

        if not isbn:
            st.warning("Please enter an ISBN to delete.")
            return

        book_to_delete = self.find_book(isbn)
        if book_to_delete:
            self.books.remove(book_to_delete)
            st.success(f"Book with ISBN {isbn} deleted successfully.")
        else:
            st.error(f"Book with ISBN {isbn} not found.")

    def delete_user(self, member_id:int):

        '''Delete a user by member ID.'''

        if not member_id:
            st.warning("Please enter a Member ID to delete.")
            return

        user_to_delete = self.find_user(member_id)
        if user_to_delete:
            self.users.remove(user_to_delete)
            st.success(f"User with ID {member_id} deleted successfully.")
        else:
            st.error(f"User with ID {member_id} not found.")

    def borrow_book(self, isbn, member_id:int):

        ''''Borrow a book by ISBN and Member ID.'''

        if not isbn or not member_id:
            st.warning("Please enter both ISBN and Member ID to borrow a book.")
            return

        user = self.find_user(member_id)
        book = self.find_book(isbn)

        if user and book:
            if not book.borrowed:
                user.borrowed_books.append(book)
                book.borrowed = True
                st.success(f"Book with ISBN {isbn} borrowed successfully.")
            else:
                st.error(f"Book with ISBN {isbn} is already borrowed.")
        else:
            st.error("User or Book not found.")

    def return_book(self, isbn:int, member_id:int):

        '''Return a book by ISBN and Member ID.'''

        if not isbn or not member_id:
            st.warning("Please enter both ISBN and Member ID to return a book.")
            return

        user = self.find_user(member_id)
        book = self.find_book(isbn)

        if user and book:
            if book in user.borrowed_books:
                user.borrowed_books.remove(book)
                book.borrowed = False
                st.success(f"Book with ISBN {isbn} returned successfully.")
            else:
                st.error(f"You do not have book with ISBN {isbn} borrowed.")
        else:
            st.error("User or Book not found.")

    def search_books(self, title:str):

        '''Search for books by title.'''

        if not title:
            st.warning("Please enter a title to search for books.")
            return

        matching_books = [book for book in self.books if title.lower() in book.title.lower()]
        if matching_books:
            st.success("Matching Books:")
            for book in matching_books:
                st.write(book)
        else:
            st.warning("No matching books found.")

    def check_book_availability(self, isbn:int):

        '''Check if a book is available by ISBN.'''

        if not isbn:
            st.warning("Please enter an ISBN to check availability.")
            return

        book = self.find_book(isbn)
        if book:
            if not book.borrowed:
                st.success(f"Book with ISBN {isbn} is available.")
            else:
                st.warning(f"Book with ISBN {isbn} is currently checked out.")
        else:
            st.error(f"Book with ISBN {isbn} not found.")

    def search_users(self, name:str):

        '''Search for users by name.'''

        if not name:
            st.warning("Please enter a name to search for users.")
            return

        matching_users = [user for user in self.users if name.lower() in user.name.lower()]
        if matching_users:
            st.success("Matching Users:")
            for user in matching_users:
                st.write(user)
        else:
            st.warning("No matching users found.")


library = Library()

def add_book():

    '''Add a book to the library.'''

    isbn = st.text_input("Enter ISBN:")
    title = st.text_input("Enter Title:")
    author = st.text_input("Enter Author:")

    if st.button("Add Book"):
        book = Book(isbn, title, author)
        library.add_book(book)
        st.success("Book added successfully!")

def add_user():

    ''''Add a user to the library.'''

    name = st.text_input("Enter Name:")
    member_id = st.text_input("Enter Member ID:")

    if st.button("Add User"):
        user = User(name, member_id)
        library.add_user(user)
        st.success("User added successfully!")

def export_books():
    
    '''Export the books list to a csv file (should take a filename as a parameter)'''

    filename = st.text_input("Enter the filename for exporting books (e.g., books.csv):")

    if st.button("Export Books"):
        library.export_books_to_csv(filename)
        st.success("Books exported successfully!")

def export_users():

    '''Export the users list to a csv file (should take a filename as a parameter)'''

    filename = st.text_input("Enter the filename for exporting users (e.g., users.csv):")

    if st.button("Export Users"):
        library.export_users_to_csv(filename)
        st.success("Users exported successfully!")

def delete_book():

    '''Delete a book by ISBN.'''

    isbn = st.text_input("Enter ISBN to delete:")
    library.delete_book(isbn)

def delete_user():
    
    ''''Delete a user by member ID.'''
    
    member_id = st.text_input("Enter Member ID to delete:")
    library.delete_user(member_id)

def borrow_book():

    ''''Borrow a book by ISBN and Member ID.'''

    isbn = st.text_input("Enter ISBN of the book to borrow:")
    member_id = st.text_input("Enter your Member ID:")
    library.borrow_book(isbn, member_id)

def return_book():

    '''Return a book by ISBN and Member ID.'''

    isbn = st.text_input("Enter ISBN of the book to return:")
    member_id = st.text_input("Enter your Member ID:")
    library.return_book(isbn, member_id)

def search_books():

    '''Search for books by title.'''

    title = st.text_input("Enter Title to search:")
    library.search_books(title)

def check_book_availability():

    '''Check if a book is available by ISBN.'''

    isbn = st.text_input("Enter ISBN to check availability:")
    library.check_book_availability(isbn)

def search_users():

    '''Search for users by name.'''


    name = st.text_input("Enter Name to search:")
    library.search_users(name)

def main():
    
    '''Main function to run the program.'''

    st.title("🎄Santa's Library System🎄")

    st.markdown(
        """
        <style>
            .stApp {
                background: linear-gradient(to right, #228B22, #FF0000);
                color: white;
            }
            .st-bw {
                color: #228B22;
            }
            .st-dx {
                color: #FF0000;
            }
        </style>
        """
        , unsafe_allow_html=True
    )
    menu = st.sidebar.selectbox("Select an option", ["Add Book", "Add User", "Export Books", "Export Users",
                                                     "Delete Book", "Delete User", "Borrow Book", "Return Book",
                                                     "Search Books", "Check Book Availability", "Search Users"])

    if menu == "Add Book":
        add_book()
    elif menu == "Add User":
        add_user()
    elif menu == "Export Books":
        export_books()
    elif menu == "Export Users":
        export_users()
    elif menu == "Delete Book":
        delete_book()
    elif menu == "Delete User":
        delete_user()
    elif menu == "Borrow Book":
        borrow_book()
    elif menu == "Return Book":
        return_book()
    elif menu == "Search Books":
        search_books()
    elif menu == "Check Book Availability":
        check_book_availability()
    elif menu == "Search Users":
        search_users()



if __name__ == "__main__":
    main()